...
#else
    const char* pszModule = "summercoinv2";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "summercoinv2d.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong SummerCoinV2 will not work properly.");
